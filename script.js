...
(function(){
  ...
  function joinRoom(id){
    ...
    if(roomUnsub) roomUnsub(); if(participantsUnsub) participantsUnsub();
    if(!roomId){
      renderCards(null);
  ...
  function updateStats(){
    const nums = getNumericVotes();
    const setText = (el, v) => el.textContent = (nums.length? v.toString() : '—');
    if(nums.length){
      const min = Math.min(...nums);
      const max = Math.max(...nums);
      const avg = Math.round((nums.reduce((a,b)=>a+b,0)/nums.length)*100)/100;
      setText(minValEl, min);
      setText(avgValEl, avg);
      setText(maxValEl, max);
    }else{
      setText(minValEl, '—'); setText(avgValEl, '—'); setText(maxValEl, '—');
    }
  }
